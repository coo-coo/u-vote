...
public class AccountHelper {
	 ...
	 * @since 0.1.1.0
	 */
	public static SimpleMessage<?> activeAccount(String account) {
		...
		if (list != null && list.size() <= 0) {
			...
			return new SimpleMessage<Object>(new SimpleMessageHead(
					SimpleMessageHead.REP_OK, "账号已激活"));
		}
		return new SimpleMessage<Object>(new SimpleMessageHead(
				SimpleMessageHead.REP_NOT_FOUND, account + "不存在"));
	}
	 ...
	 * @since 0.1.1.0
	 */
	public static SimpleMessage<?> lockAccount(String account) {
		...
		if (list != null && list.size() > 0) {
			...
			VoteUtil.getMongo().updateFull(Account.C_NAME, _id, map);
			return new SimpleMessage<Object>(new SimpleMessageHead(
					SimpleMessageHead.REP_OK, account + "被锁定"));
		}
		return new SimpleMessage<Object>(new SimpleMessageHead(
				SimpleMessageHead.REP_NOT_FOUND, account + "不存在"));
	}
	 ...
	 * @since 0.6.0.0
	 */
	public static SimpleMessage<?> focus(String account, String subject,
			String type) {
	 ...
	 * @since 0.6.0.0
	 */
	public static SimpleMessage<?> unfocus(String account, String subject,
			String type) {
		// 判断focus是否存在
	 ...
	 * @since 0.5.2.0
	 */
	public static boolean accountPwdreset(String account, String password) {
		// 通过uuid找到对象，再更新为新对象
		QueryAttrs query = QueryAttrs.blank().add("mobile", account);
